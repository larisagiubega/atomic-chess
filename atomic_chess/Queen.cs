...
namespace atomic_chess
{
    class Queen : Piece, IPiece
    {
        ...
        public Queen(int x, int y, Piece.Color color, string name) : base(x, y, color, name)
        {
            points = 9;
        }
        ...
        public override bool canMove(int newX, int newY, Piece.Color color)
        {
            ...
            for (int i = 0; i < 8; i++)//parcurgem matricea de mutari legale
            {
                x = this.x;//la fiecare iteratie x ia x initial al obiectului
                y = this.y;//la fiecare iteratie y ia y initial al obiectului

                while (x + legalMoves[i, 0] >= 0 && x + legalMoves[i, 0] <= 7 &&
                    y + legalMoves[i, 1] >= 0 && y + legalMoves[i, 1] <= 7) //cat timp nu s-a iesit din tabla cu x si cu y
                {
